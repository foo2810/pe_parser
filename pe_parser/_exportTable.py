...
from pe_parser._peBaseClass import *
...
class ExportTable:
	...
	def isExported(self, addr):
		return addr >= self.headRva and addr < self.headRva + self.size
	...
	def __iter__(self):
		for i, j in zip(self.exportNameOrdinalTable, self.exportNameTable):
			yield (i, j)
